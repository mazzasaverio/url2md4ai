...
from .utils import get_logger, SimpleCache
...
class ConversionResult:
    ...
    @classmethod
    def success_result(cls, **kwargs) -> "ConversionResult":
...
class ContentCleaner:
    ...
    def clean_with_trafilatura(self, html_content: str, url: str) -> Optional[str]:
        """Extract clean content using trafilatura."""
        if not TRAFILATURA_AVAILABLE:
            self.logger.warning("Trafilatura not available, falling back to BeautifulSoup")
            return None
        ...
        try:
            ...
            if extracted and self.config.llm_optimized:
                return self._post_process_for_llm(extracted)
    ...
    def _remove_unwanted_elements(self, soup: BeautifulSoup) -> BeautifulSoup:
        ...
        if self.config.remove_ads:
            unwanted_selectors.extend([
                '[id*="ad"]', '[class*="ad"]',
                '[id*="advertising"]', '[class*="advertising"]',
    ...
    def _remove_cookie_heavy_content(self, soup: BeautifulSoup):
        """Remove elements that are heavily cookie-related."""
        cookie_keywords = [
            'cookie', 'consent', 'gdpr', 'privacy policy', 'tracking',
            'necessary cookies', 'marketing cookies', 'analytics cookies',
            'cookiebot', 'cookie policy', 'cookie consent', 'data processing',
            'third party', 'advertising partners', 'personalization'
        ]
        ...
        for element in soup.find_all(['div', 'section', 'article', 'p']):
            if element.get_text():
                ...
                if len(text_content) > 50:  # Only check substantial text blocks
                    cookie_mentions = sum(1 for keyword in cookie_keywords if keyword in text_content)
    ...
    def _post_process_for_llm(self, content: str) -> str:
        """Post-process content for optimal LLM consumption."""
        ...
        for line in lines:
            ...
            cookie_score = sum(1 for word in ['cookie', 'consent', 'gdpr', 'privacy', 'tracking'] 
                             if word in line_lower)
            word_count = len(line_lower.split())
            
            if word_count > 5 and cookie_score / word_count < 0.3:  # Less than 30% cookie words
                filtered_lines.append(line)
...
class URLToMarkdownConverter:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.logger.info(f"Converter initialized with config")
        if not TRAFILATURA_AVAILABLE and self.config.use_trafilatura:
            self.logger.warning("Trafilatura not available, using BeautifulSoup fallback")
    ...
    async def convert_url(
        ...
        try:
            ...
            use_js = use_javascript if use_javascript is not None else self.config.javascript_enabled
            use_traff = use_trafilatura if use_trafilatura is not None else self.config.use_trafilatura
            ...
            if use_traff and TRAFILATURA_AVAILABLE:
                ...
                if not markdown:
                    self.logger.info("Trafilatura failed, falling back to BeautifulSoup")
            ...
            if self.config.use_hash_filenames:
                ...
                filename = f"{safe_title.replace(' ', '_')}.md" if safe_title else URLHasher.generate_filename(url)
            ...
            self.logger.info(f"Conversion completed: {len(markdown)} chars via {extraction_method}")
            return result
    ...
    async def _fetch_content(self, url: str, use_javascript: bool) -> Tuple[str, Dict]:
    ...
    async def _fetch_with_playwright(self, url: str) -> Tuple[str, Dict]:
    ...
    def convert_url_sync(self, url: str, **kwargs) -> ConversionResult:
