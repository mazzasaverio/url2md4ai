...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            use_hash_filenames=os.getenv("URL2MD_USE_HASH_FILENAMES", "true").lower() == "true",
            ...
            wait_for_network_idle=os.getenv("URL2MD_WAIT_NETWORK", "true").lower() == "true",
            page_wait_timeout=int(os.getenv("URL2MD_PAGE_TIMEOUT", "2000")),
            
            # Content extraction settings
            use_trafilatura=os.getenv("URL2MD_USE_TRAFILATURA", "true").lower() == "true",
            clean_content=os.getenv("URL2MD_CLEAN_CONTENT", "true").lower() == "true",
            llm_optimized=os.getenv("URL2MD_LLM_OPTIMIZED", "true").lower() == "true",
            
            # Content filtering settings
            remove_cookie_banners=os.getenv("URL2MD_REMOVE_COOKIES", "true").lower() == "true",
            remove_navigation=os.getenv("URL2MD_REMOVE_NAV", "true").lower() == "true",
            remove_ads=os.getenv("URL2MD_REMOVE_ADS", "true").lower() == "true",
            remove_social_media=os.getenv("URL2MD_REMOVE_SOCIAL", "true").lower() == "true",
            remove_comments=os.getenv("URL2MD_REMOVE_COMMENTS", "true").lower() == "true",
            
            # Advanced trafilatura settings
            favor_precision=os.getenv("URL2MD_FAVOR_PRECISION", "true").lower() == "true",
            ...
            include_images=os.getenv("URL2MD_INCLUDE_IMAGES", "false").lower() == "true",
            include_comments=os.getenv("URL2MD_INCLUDE_COMMENTS", "false").lower() == "true",
            include_formatting=os.getenv("URL2MD_INCLUDE_FORMATTING", "true").lower() == "true",
            ...
            log_format=os.getenv("URL2MD_LOG_FORMAT", "%(asctime)s - %(name)s - %(levelname)s - %(message)s"),
        )
